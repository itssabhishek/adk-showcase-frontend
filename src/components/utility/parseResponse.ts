/**
 * Utility function to extract text from a WebSocket message response
 * Handles parsing the JSON string and extracting the text content
 */
 ...
 * @param data - The raw response data as a string or parsed object
 * @returns The extracted text content or null if extraction fails
 */
export function extractMessageText(data: string | object): string | null {
  try {
    ...
    if (typeof data === 'string') {
      ...
      try {
        ...
        // If standard parsing fails, try replacing single quotes with double quotes
        // This handles the case when the string uses single quotes instead of double quotes
        const fixedJson = data.replace(/['`]/g, '"');
        try {
          parsed = JSON.parse(fixedJson);
        } catch (innerError) {
          console.error('[extractMessageText] Error parsing fixed JSON:', innerError);
          return null;
        }
      }
    } else {
...
/**
 * Parses a WebSocket message and returns the full structured response
 * @param data - The raw response data as a string
 * @returns The parsed response object or null if parsing fails
 */
